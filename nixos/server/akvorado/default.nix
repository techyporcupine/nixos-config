...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.akvorado-orchestrator = {
      ...
      serviceConfig = {
        ExecStart = "${akvoradoPkg}/bin/akvorado --debug orchestrator /var/lib/akvorado-orchestrator/config.yaml";
        Restart = "always";
        DynamicUser = true;
    ...
    systemd.services.akvorado-inlet = {
      ...
      serviceConfig = {
        ExecStart = "${akvoradoPkg}/bin/akvorado --debug inlet http://127.0.0.1:8083";
        Restart = "always";
        DynamicUser = true;
    ...
    systemd.services.akvorado-console = {
      ...
      serviceConfig = {
        ExecStart = "${akvoradoPkg}/bin/akvorado --debug console http://127.0.0.1:8083";
        Restart = "always";
        DynamicUser = true;
